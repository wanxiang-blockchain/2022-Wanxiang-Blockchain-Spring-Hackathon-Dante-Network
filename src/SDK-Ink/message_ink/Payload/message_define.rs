...
/// SQOS structure. Details can be seen in the [technology white paper, chapter 3.1](https://github.com/dantenetwork/Pitch-Deck)
/// @Reveal: SQoS item `Hidden & Reveal`
/// @Challenge: SQoS item `Challenge Confirmation`
/// @Threshold: SQoS item `Verification Threshold`
/// @Priority: SQoS item `Priority:`
/// @ExceptionRollback: SQoS item `Exception rollback`
/// @Anonymous: SQoS item `Anonymous transactions`
/// @Identity: SQoS item `Identity traceability`
/// @Isolation: SQoS item `Send/receive isolation`
/// @CrossVerify: SQoS item `Cross-verification`
...
/// @id: Message id, like `nonce in transaction`
/// @from_chain: the chain received messages from
/// @sender: message sender in the source chain
/// @signer: message signer in the source chain
/// @sqos: Security quality of service
/// @contract: application `target contract` on the target chain
/// @action: the interface of the `target contract`, which is the `selector` in ink!
/// @data: message payload
/// @session: cross-chain message communication session
...
/// @session: corss-chain communication session
#[derive(Decode, Encode, Clone)]
// #[cfg_attr(feature = "std", derive(scale_info::TypeInfo))]
